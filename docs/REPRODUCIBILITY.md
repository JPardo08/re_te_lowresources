...
`data/final/process_statistics.csv` is derived automatically from the canonical platform, Selection, manual, and final CSVs (same spirit as Paper 2). It stores long-format counts and, where useful, percentages. It is regenerated by `reproduce_all.py` / Selection and checked by `validate_reproduction.py`. Free / Peer-review stages are not modeled.
